...
- 🎨 **Visual Feedback**:
  ...
  - **Right-click indicator to pause/unpause** timer quickly
  - **Keyboard shortcut to hide/show indicator** (default: Alt+Shift+H)
...
- ✅ **Lag-Resilient Countdown**: Timer and Distraction Dump now decrement using wall-clock elapsed time, preventing apparent pauses during browser lag.
- ✅ **Heartbeat Stabilization**: Active owner windows now update cross-window heartbeat on a wall-clock cadence to reduce ownership flapping.
- ✅ **Safer State Restore**: Timer and dump restore paths now validate malformed persisted fields and clamp custom cycle indices to safe values.
...
- ✅ **Window Ownership Claim**: Cross-window ownership is claimed before dump start/end for proper sync handling
- 🎨 **Export Logs Location**: Moved "Export Logs" button from settings dialog to main Pomodoro menu for easier access

### Version 1.4.7

- ✅ **Sine v2.3 Compatibility**: Fixed `scripts` metadata key format (changed from URL to local file path: `"zen-pomodoro-focus-blocker.uc.js": {}`) while retaining legacy `js` field for backward compatibility
...
- ✅ Fixed "End Break Early" button not working during transition phase in custom cycles
- ✅ Fixed Distraction Dump workspace blocking - all workspaces now properly unblocked during dump
- ✅ Fixed code lockout screen character alignment (input text now aligns with displayed code)
- 📝 Updated documentation with v1.3.9 changes
